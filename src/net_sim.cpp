...
int main(int argc, char** argv) {
    ...
    if (argc >= 3) { width = (uint32_t)atoi(argv[1]); height = (uint32_t)atoi(argv[2]); }
    double framePixels = (double)width * (double)height;
    uint32_t reps = (uint32_t)llround(framePixels / (512.0 * 512.0));
    if (reps < 1) reps = 1;
